...
@pytest.mark.parametrize(
    "distance,expected",
    (
        (Distance.DOT, [[14, 0, 2], [0, 0, 0]]),
        (Distance.COSINE, [[1, 0, 2 / 42**0.5], [0, 0, 0]]),
        (Distance.EUCLIDEAN, [[0, 14**0.5, 13**0.5], [14**0.5, 0, 3**0.5]]),
    ),
)
def test_scores_calculator(distance: Distance, expected: tp.List[tp.List[float]]) -> None:
    subject_factors = np.array(
        [
            [1, -2, 3],
            [0, 0, 0],
        ]
    )
    object_factors = np.array(
        [
            [1, -2, 3],
            [0, 0, 0],
            [1, 1, 1],
    ...
    calculator = ScoreCalculator(distance, subject_factors, object_factors)
    actual = np.array([calculator.calc(i) for i in (0, 1)])
    np.testing.assert_almost_equal(actual, expected)


def test_scores_calculator_with_incorrect_distance() -> None:
    ...
    def test_with_incorrect_distance(self, method: str) -> None:
        with pytest.raises(ValueError):
            if method == "u2i":
                ...
                user_vectors, item_vectors = m._get_u2i_vectors(self.stub_dataset)
            else:
                m = self.make_model(self.user_biased_factors, self.item_biased_factors, i2i_distance=7)  # type: ignore
                item_vectors_1, item_vectors_2 = m._get_i2i_vectors(self.stub_dataset)

...
def cross_validate_hstu(# pylint: disable=too-many-locals
    proxy_recommend,
    dataset: Dataset,
    splitter: Splitter,
    metrics: tp.Dict[str, MetricAtK],
    ...
    split_infos = []
    metrics_all = []
    models_require_time_supervision = ["hstu"]
    for train_ids, test_ids, split_info in split_iterator:
        split_infos.append(split_info)
        ...
        test_users = interactions_df_test[Columns.User].unique()
        #print("test_users_len: ",len(test_users) )
        prev_interactions = fold_dataset.get_raw_interactions()
        catalog = prev_interactions[Columns.Item].unique()
        ...
        for model_name in ref_models or []:
            ...
            if model_name in models_require_time_supervision:
                df_test = interactions_df_test
            else:
                df_test = None
                print(model_name)
            ref_reco[model_name] = proxy_recommend(model = model,
                users=test_users,
                dataset=fold_dataset,
                k=k,
                filter_viewed=filter_viewed,
        ...
        for model_name, model in models.items():
            ...
            if model_name in ref_reco:
                ...
                model.fit(fold_dataset)
                reco = proxy_recommend(model = model,
                    users=test_users,
                    dataset=fold_dataset,
                    k=k,
                    filter_viewed=filter_viewed,
